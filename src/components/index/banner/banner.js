...
class Banner extends React.Component{
  ...
  render() {
    ...
    let bannerList = bannerDiv.map((value, index) => {
      dd.push(
        <li className={`contrdd ${this.state.currentPage === index ? 'is-active': ''}`}
          data-index={this.state.current > 0 ? '11' : '222'}
          ref={(li) => {this.li = li}}
          key={index}
        >
          {this.state.current}
    ...
    return (
      <div className="banner">
        <ReactSwipe className="carousel" ref={
          (reactSwipe) => {
